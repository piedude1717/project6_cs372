...
def ipv4_to_value(ipv4_addr):
    ...
    # TODO -- write me!
    nums = ipv4_addr.split(".")
    nums = [eval(i) for i in nums]
    nums_hex = ''
    for i in range(len(nums)):
        nums_hex += str("0x{:02x}".format(nums[i]))
    nums_hex = nums_hex.split("0x")
    nums_hex = "0x" + ''.join(nums_hex)
    nums_bin = "{0:08b}".format(int(nums_hex, 16))
    nums_dec = int(nums_bin, 2)
    answer = nums_hex + ' ' + nums_bin + ' ' + str(nums_dec)
    return answer
...
def value_to_ipv4(addr):
    ...
    # TODO -- write me!
    addr_hex = addr[2:10]
    addr_hex_split = str(int(addr_hex[0:2], 16)) + "." + str(int(addr_hex[2:4], 16)) + "." + str(int(addr_hex[4:6], 16)) + "." + str(int(addr_hex[6:8], 16))
    return str(addr_hex_split)
...
def get_subnet_mask_value(slash):
    ...
    # TODO -- write me!
    bin_num = '0b'
    place_holder = slash.split('/')
    count_for_1s = place_holder[1]
    count_for_0s = 32 - int(count_for_1s)
    for i in range(int(count_for_1s)):
        bin_num += '1'

    for i in range(int(count_for_0s)):
        bin_num += '0'

    hex_num = hex(int(bin_num, 2))
    dec_num = int(bin_num, 2)
    answer = hex_num + ' ' + bin_num + ' ' + str(dec_num)
    return answer
...
def ips_same_subnet(ip1, ip2, slash):
    ...
    subnet_ip_dec = get_subnet_mask_value(slash)
    subnet_ip_dec = subnet_ip_dec[len(subnet_ip_dec)-10:len(subnet_ip_dec)]
    subnet_ip_dec = int(subnet_ip_dec)
    ip1_dec = ipv4_to_value(ip1)
    ip1_dec = ip1_dec[len(ip1_dec)-10:len(ip1_dec)]
    ip1_dec = int(ip1_dec)
    ip1_subnet = bin(subnet_ip_dec & ip1_dec)
    ip2_dec = ipv4_to_value(ip2)
    ip2_dec = ip2_dec[len(ip2_dec) - 10:len(ip2_dec)]
    ip2_dec = int(ip2_dec)
    ip2_subnet = bin(subnet_ip_dec & ip2_dec)
...
def find_router_for_ip(routers, ip):
    ...
    # TODO -- write me!
    for key in routers:
        slash = routers[key]["netmask"]
        if ips_same_subnet(key, ip, slash) == True:
            return key

        elif key == list(routers)[-1]:
            return None
...
"""
ip = "10.190.20.66"
print(ipv4_to_value(ip))

hex_val = str(hex(180229186))
print(value_to_ipv4(hex_val))
## -------------------------------------------
